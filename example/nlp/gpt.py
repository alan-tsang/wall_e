...
def collate_fn(data):
    ...
    attention_mask = causal_mask[:, :max_len, :max_len] & pad_mask.unsqueeze(1)
    # attention_mask = torch.ones_like(input_ids).unsqueeze(1)
...
class GPT(BaseModel):
    def __init__(self, cfg, tokenizer):
        super().__init__()
        self.transformer = registry.get_model_class(cfg.model.type)(**(dict(cfg.model) | {"vocab_n": len(tokenizer)}))
        self.logit_generator = nn.Linear(cfg.model.d, len(tokenizer))
    ...
    def forward(self, input_ids, attention_mask = None, past_key_values = None, 
        ...
        logits_ids = self.logit_generator(hidden_states)

        return dict(logits_ids = logits_ids, past_key_values = next_past)

    def train_step(self, data_batch):
        input_ids = data_batch["input_ids"]
        mask = data_batch["attention_mask"]
    ...
    def test_step(self, data_batch, **kargs):
        ...
        # 该函数返回一个dict(generated_ids = **)的字典，需要exact_smiles
