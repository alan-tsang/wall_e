...
class RotaryMultiDotProductAttention(nn.Module):
    """
    ...
    def forward(self, q, k, v, attention_mask, past_key_value = None):
        ...
        return (self.projs[-1](self.dropout(qkv)), (kw, vw)), self.weight

    def mask_softmax_dropout(self, qk, mask):
        """
        Args:
            qk: (b, n, l_q, l_k)  -- q(b, n, l_q, d) * k(b, n, l_k, d).T -> (b, n, l_q, l_k)
            mask: (b, l_q, l_k) Boolean for cross-attention or (b, l) for self-attention
        """
        if mask is None:
        ...
        if mask.dim() == 2:
            # Self-attention: (b, l) -> (b, 1, l, l)
            # For self-attention, we need to create a square mask
            l = mask.size(1)
            mask = mask.unsqueeze(1).unsqueeze(1)  # (b, 1, 1, l)
            mask = mask & mask.transpose(-1, -2)  # (b, 1, l, l)
        elif mask.dim() == 3:
            # Cross-attention: (b, l_q, l_k) -> (b, 1, l_q, l_k)
            mask = mask.unsqueeze(1)  # (b, 1, l_q, l_k)
        elif mask.dim() == 4:
            # Already in correct format: (b, 1, l_q, l_k)
            pass
        else:
...
if __name__ == '__main__':
    ...
    print(f"{mask=}")
    attention = RotaryMultiDotProductAttention(2, 512)
