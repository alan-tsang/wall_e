...
class ManagerMeta(type):
    ...
    1. The subclasses inheriting from ``ManagerMeta`` will manage their
       own ``_instance_dict`` and root instances.
    2. The constructors of subclasses must contain the ``name`` argument.
...
if __name__ == '__main__':
    class Demo(ManagerMixin):
        def __init__(self, name = ''):
            super().__init__(name)


    x = Demo.get_instance('name1')
    print(x.instance_name)
    y = Demo.get_instance('name1')
    print(id(x) == id(y))
